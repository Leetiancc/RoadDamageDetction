function [E_yij, E_inv_yij] = E_step2(Y, init_a, TNi, model, E_yij_half, eta, n)
    ...
    for j =1:length(y)
        ...
        if j == 1
            ...
            elseif abs(t(j) - t_int*5) < epsilon
                ...
                E_yij_obs{i} = E_yij_half{i}(j);
                E_inv_yij_obs{i} = [1 / E_yij_half{i}(j)];
                negative_num = [1 / E_yij_half{i}(j)];
                if any(negative_num < 0)
                    warning('第%d条退化路径，第%d时间点有负数',i,j)
                end
        ...
        elseif j >= 2 && j <= length(y)
            ...
            elseif abs(t(j) - t(j-1) - t_int*5) < epsilon
                ...
                E_yij_obs{i} = [E_yij_obs{i}, E_yij_half{i}(j)];
                E_inv_yij_obs{i} =[E_inv_yij_obs{i}, 1 / E_yij_half{i}(j)];
                if any(1 / E_yij_half{i}(j) < 0)
                    warning('第%d条退化路径，第%d时间点有负数',i,j)
                end
